...
bool UML_WavePropagationSubsystem::UndoLastAction_Animated()
{
	...
	if (Action.Type == EML_UndoActionType::PlantWaves)
	{
		...
		// In the current flow, planting cost is paid BEFORE BeginTurnRecord_Internal().
		// So EnergyBefore is already the post-cost value, and undo must refund 1 here.
		PlayerController->CurrentEnergy = ActiveUndoRecord.EnergyBefore + 1;
